...
contract MainGovernance is Ownable, ReentrancyGuard {
    ...
    function createProposal(
        string memory _title, string memory _description, uint256 _durationMinutes,
    ...
    function collectSecondaryChainVotes(
        bytes32 proposalId, string memory chainId, uint256 yesVotes, int256 noVotes
    ...
    function finalizeProposalVotes(bytes32 proposalId) external {
        ...
        require(proposal.startTime > 0, "Proposal doesn't exist");
        require(
            block.timestamp >= proposal.endTime + 1 minutes, "Cooldown period not ended"
        );
